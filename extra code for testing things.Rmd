...
I am expecting the values of this variable to be a float between 0 and 1. There are no observations with invalid values for this variable

```{r}
plot(data_news$global_rate_positive_words, data_news$shares, xlab = "Rate of Positive Words in Article", ylab = "Number of Times Article was Shared")

range(data_news$global_rate_positive_words)
```
...
```{r}
plot(data_news$global_rate_negative_words, data_news$shares, xlab = "Rate of Negative Words in Article", ylab = "Number of Times Article was Shared")

range(data_news$global_rate_negative_words)
```
...
```{r}
plot(data_news$rate_positive_words, data_news$shares, xlab = "Rate of Positive Words Among Non-neutral Tokens", ylab = "Number of Times Article was Shared")

range(data_news$rate_positive_words)
```
...
```{r}
plot(data_news$rate_negative_words, data_news$shares, xlab = "Rate of Negative Words Among Non-neutral Tokens", ylab = "Number of Times Article was Shared")

range(data_news$rate_negative_words)
```
...
1. ntokenstitle: the number of words in the news article's title (Integer)
2. ntokenscontent: the number of words in the entire news article (Integer)
3. nuniquetokens: Rate of unique words in the news article (Integer)
4. nnonstop_words: Rate of non-stop words in the news article (Integer)
5. nnonstopuniquetokens: Rate of unique non-stop words in the content(Integer)
6. num_hrefs: Number of links (Integer)
7. numselfhrefs: Number of links to other articles published by Mashable (Integer)
8. num_imgs: Number of images in the article(float)
9. num_videos: Number of videos in the article (float)
10. averagetokenlength: Average length of the words in the article (float)
11. num_keywords: Number of keywords in the metadata (float)
12. datachannelis_lifestyle: Was the article was posted in the data channel called 'Lifestyle'? (Binary)
13. datachannelis_entertainment: Was the article was posted in the data channel called 'Entertainment'? (Binary)
14. datachannelis_bus: Was the article was posted in the data channel called 'Business'? (Binary)
15. datachannelis_socmed: Was the article was posted in the data channel called 'Social Media'? (Binary)
16. datachannelis_tech: Was the article was posted in the data channel called 'Tech'? (Binary)
17. datachannelis_world: Was the article was posted in the data channel called 'World'? (Binary)

For variables 18-26, the researchers ranked the keywords in the article by the number of times articles with those keywords got shared on average (known before publication). Then they were able to find the worst keyword, average keyword, and best keyword. For each keyword, they listed the minimum number of times articles with that keyword were shared, the maximum number of times articles with that keyword were shared, and the average number of times articles with that keyword were shared. 

18. kwminmin: Worst keyword (min. shares) (float)
19. kwmaxmin: Worst keyword (max. shares) (float)
20. kwavgmin: Worst keyword (avg. shares) (float)
21. kwminmax: Best keyword (min. shares) (float)
22. kwmaxmax: Best keyword (max. shares) (float)
23. kwavgmax: Best keyword (avg. shares) (float)
24. kwminavg: Avg. keyword (min. shares) (float)
25. kwmaxavg: Avg. keyword (max. shares) (float)
26. kwavgavg: Avg. keyword (avg. shares) (float)
...
It appears that each article was only classified into one category out of business, entertainment, lifestyle, socmed, tech and world. For example, there were no articles classified as both business and tech articles. Thus I will combine the binary variables denoting article category into a single 6-level factor variable as follows

1. 1 - business article
2. 2 - entertainment article
3. 3 - lifestyle article
4. 4 - socmed article
5. 5 - tech article
6. 6 - world article
...
```{r}
data_news %>% mutate(channel_sum = data_channel_is_bus + data_channel_is_entertainment + data_channel_is_lifestyle + data_channel_is_socmed + data_channel_is_tech + data_channel_is_world) %>% filter(channel_sum > 1)
```

```{r}
data_news <- data_news %>% mutate(channel = ifelse(data_channel_is_bus == 1, 1, 
                                      ifelse(data_channel_is_entertainment == 1, 2,
                                             ifelse(data_channel_is_lifestyle == 1, 3,
                                                    ifelse(data_channel_is_socmed == 1, 4,
                                                           ifelse(data_channel_is_tech == 1, 5,
                                                                  ifelse(data_channel_is_world, 6, NA))))))) %>%
...
Furthermore it is possible that the number of shares for some news articles was correlated, due to the articles covering the same viral event. Articles can also contain links to other articles, so articles referencing each other may have their number of shares being correlated.

There were also likely random effects due to randomness in when/how readers read the news articles, readers' interests in different article topics, and whether readers were in the mood to share the articles.

Thus a mixed effects model may be appropriate.

We note that from the summary statistics, the explanatory variables all have ranges between -1 and 1. But the response variable, the number of shares, is in the thousands. So the scale of the response variable and explanatory variables are very different, and transformations may be needed on the response variable.

```{r eval=FALSE, fig.width=5, include=FALSE}
library(knitr)
data2014sampled2 %>% select(-url) -> data2014sampled3
s <- summary(data2014sampled3)

kable(t(s), caption = "Summary Statistics of Variables", format = "pipe")
```